...
/* inspiration from xavier aguilar */
...
struct prev_based_storage
{
	...
	uint32_t nr_closed_event_flows;
...
struct per_thread_region_info
{
	...
	uint32_t * avail_prevs;
	uint32_t nr_avail_prevs;

	uint64_t total_visits;
};

/*
 * TODO get rid of the per_thread thingy here and include it in struct thread_info
 */
 ...
 * */
static inline void append_to_closed_flows(
		struct prev_based_storage * region_thread_info, int event_flow_index)
{
	/* move rule to closed rules */

	/* extend closed by 1 */
	region_thread_info->closed_event_flows = realloc(
			region_thread_info->closed_event_flows,
			(region_thread_info->nr_closed_event_flows + 1)
			* sizeof(struct temporal_event_flow *));
	/* copy reference */
	region_thread_info->closed_event_flows[region_thread_info->nr_closed_event_flows] =
			region_thread_info->open_event_flows[event_flow_index];
	...
	memmove(&region_thread_info->open_event_flows[event_flow_index],
			&region_thread_info->open_event_flows[event_flow_index + 1],
			(region_thread_info->nr_open_eventflows - event_flow_index - 1)
			* sizeof(struct temporal_event_flow *));
 ...
 * */
static inline void clean_up_closed_flows(
		struct prev_based_storage * region_thread_info)
{
	/* fold */
	for (uint32_t i = 0; i < region_thread_info->nr_closed_event_flows; i++)
	{
		struct temporal_event_flow * current = region_thread_info->closed_event_flows[i];
		for (uint32_t j = i + 1; j < region_thread_info->nr_closed_event_flows; j++)
		{
			struct temporal_event_flow * current2 =
					region_thread_info->closed_event_flows[j];
			if ((current2->follow_stack_x == current->follow_stack_x)
					&& (current2->previous_stack_x == current->previous_stack_x)
					&& (current2->stride == current->stride)
					&& (current2->last_enter - current2->first_enter
							== current->last_enter - current->first_enter))
			{
				current->additional_first_enters = realloc(
						current->additional_first_enters,
						(current->nr_additional_first_enters + 1) * sizeof(int64_t));
				current->additional_first_enters[current->nr_additional_first_enters] =
						current2->first_enter;

				/* store metrics total, min, and max */
				...
				memmove(&region_thread_info->closed_event_flows[j],
						&region_thread_info->closed_event_flows[j + 1],
						(region_thread_info->nr_closed_event_flows - 1 - j)
						* sizeof(struct temporal_event_flow *));
...
/**
 * on each enter (and maybe exit), this is called.
 * here we check whether the successor rule is already known.
 */
static void insert_successor_rule(struct thread_info * thread_info,
		struct region_entry * next_region, int is_exit, uint64_t * metrics)
{
	int mapped = 0;

	uint64_t stack_x = thread_info->last_stack_x;
	uint64_t stack_x_follow = get_stack_x(thread_info, thread_info->depth)
			+ is_exit;

	/* this is tored for later. the now current stack will be the last stack in the next call */
	...
	struct per_thread_region_info * per_thread_info =
			&(region->per_thread[thread_info->id]);

	struct prev_based_storage * prev_info = NULL;

	/* check whether the last stack has been seen before */
	...
	/* if the last stack was not seen before allocate data structures for it */
	if (prev_info == NULL)
	{
		per_thread_info->avail_prevs = realloc(per_thread_info->avail_prevs,
				(per_thread_info->nr_avail_prevs + 1) * sizeof(uint64_t));
		per_thread_info->avail_prevs[per_thread_info->nr_avail_prevs] = stack_x;
		per_thread_info->prev_info = realloc(per_thread_info->prev_info,
				(per_thread_info->nr_avail_prevs + 1)
				* sizeof(struct prev_based_storage *));
		per_thread_info->prev_info[per_thread_info->nr_avail_prevs] = calloc(1,
				sizeof(struct prev_based_storage));
...
#endif
	/* the occurence of the last stack is counted now, the one of the current stack will be counted on the next call */
	prev_info->visits++;
	/* visits is the numbers of visits to the previous stack */
	uint64_t visits = prev_info->visits;

	/* first check whether we know the current region in currently open event flow rules,
	 * we just go linearly through all open event flows and check the follow ups
	 * We prefer rules that are new. thus, we do the search descending. */
	for (int32_t i = prev_info->nr_open_eventflows - 1; i >= 0; i--)
	{
		...
		{
			/* if we found a stride rule for this */
			if (current->stride != 0)
			{
				/* if it is the expected stride */
				if ((current->stride + current->last_enter) == visits)
				{
					/* store the number of calls to the last region */
					current->last_enter = visits;
					/* store the total number of calls. later we can check if this event flow can be reached at all */
					current->last_total_index = thread_info->num_regions;
					for (uint32_t j = 0; j < num_metrics; j++)
					{
						if ((metrics[j] - thread_info->last_metrics[j])
								< current->metric_min[j])
							current->metric_min[j] = metrics[j]
															 - thread_info->last_metrics[j];
						if ((metrics[j] - thread_info->last_metrics[j])
								> current->metric_max[j])
							current->metric_max[j] = metrics[j]
															 - thread_info->last_metrics[j];
						current->metric_tot[j] += metrics[j]
														  - thread_info->last_metrics[j];
					}
					mapped = 1;
					break;
			...
			/* there is currently no stride. set one :) */
			else
			{
				/* store the number of calls to the last region */
				current->last_enter = visits;
				/* diff between the last call and the current call */
	...
	/* if the current pair of region and next_region could not be mapped to an existing rule, mapped == 0*/
	...
	if (!mapped)
	{
		prev_info->open_event_flows = realloc(prev_info->open_event_flows,
				(prev_info->nr_open_eventflows + 1)
				* sizeof(struct temporal_event_flow *));
		prev_info->open_event_flows[prev_info->nr_open_eventflows] = calloc(1,
				sizeof(struct temporal_event_flow));

		prev_info->open_event_flows[prev_info->nr_open_eventflows]->previous_region =
				thread_info->last_region;
		prev_info->open_event_flows[prev_info->nr_open_eventflows]->follow_region =
				next_region->regionHandle;

		prev_info->open_event_flows[prev_info->nr_open_eventflows]->first_enter =
				visits;
		prev_info->open_event_flows[prev_info->nr_open_eventflows]->last_enter = visits;
		prev_info->open_event_flows[prev_info->nr_open_eventflows]->last_total_index =
				thread_info->num_regions;
		prev_info->open_event_flows[prev_info->nr_open_eventflows]->first_total_index =
				thread_info->num_regions;

		prev_info->open_event_flows[prev_info->nr_open_eventflows]->follow_exit =
				is_exit;
		prev_info->open_event_flows[prev_info->nr_open_eventflows]->previous_exit =
				thread_info->last_is_exit;
		prev_info->open_event_flows[prev_info->nr_open_eventflows]->previous_stack_x =
				stack_x;
		prev_info->open_event_flows[prev_info->nr_open_eventflows]->follow_stack_x =
				stack_x_follow;

		for (uint32_t j = 0; j < num_metrics; j++)
		{
			prev_info->open_event_flows[prev_info->nr_open_eventflows]->metric_min[j] =
					metrics[j] - thread_info->last_metrics[j];
			prev_info->open_event_flows[prev_info->nr_open_eventflows]->metric_max[j] =
					metrics[j] - thread_info->last_metrics[j];
			prev_info->open_event_flows[prev_info->nr_open_eventflows]->metric_tot[j] =
					metrics[j] - thread_info->last_metrics[j];
		}
 ...
 */
static void graphviz_enter(struct SCOREP_Location* location,
		...
		uint64_t* metricValues)
{
	...
	{
		struct thread_info * ti = (struct thread_info *) functions->SCOREP_Location_GetData( location, substrate_id );
 ...
 */
static void graphviz_exit(struct SCOREP_Location* location,
		__attribute__((unused)) uint64_t timestamp,
		SCOREP_RegionHandle regionHandle,
		uint64_t* metricValues)
{
		...
		SCOREP_MetricSourceType sourceType)
{
	if (num_metrics == 16)
		return;
	if (mode != SCOREP_METRIC_MODE_ACCUMULATED_START
			|| sourceType != SCOREP_METRIC_SOURCE_TYPE_PAPI)
	{
...
static void graphviz_define_region(SCOREP_RegionHandle regionId,
 ...
 */
static void graphviz_new_definition_handle(SCOREP_AnyHandle handle,
		SCOREP_HandleType type)
{
	/* on region definition, store the name and get the paradigm, also define it */
	if (type == SCOREP_HANDLE_TYPE_REGION)
	{
		const char * name = functions->SCOREP_RegionHandle_GetName(handle);
		SCOREP_ParadigmType par = functions->SCOREP_RegionHandle_GetParadigmType(
				handle);
		SCOREP_RegionType type = functions->SCOREP_RegionHandle_GetType(handle);
		graphviz_define_region(handle, name, par, type);
	}
	if (type == SCOREP_HANDLE_TYPE_SAMPLING_SET)
	{
		if (functions->SCOREP_SamplingSetHandle_GetMetricOccurrence(handle)
				== SCOREP_METRIC_OCCURRENCE_SYNCHRONOUS_STRICT
				&& functions->SCOREP_SamplingSetHandle_GetSamplingSetClass(handle)
				== SCOREP_SAMPLING_SET_CPU)
		{
			int i = 0;
			const SCOREP_MetricHandle * m_handles =
					functions->SCOREP_SamplingSetHandle_GetMetricHandles(handle);
			for (i = 0;
					i < functions->SCOREP_SamplingSetHandle_GetNumberOfMetrics(handle);
					i++)
			{
				const char* name = functions->SCOREP_MetricHandle_GetName(m_handles[i]);
				SCOREP_MetricMode mode = functions->SCOREP_MetricHandle_GetMode(
						m_handles[i]);
				SCOREP_MetricSourceType srct =
						functions->SCOREP_MetricHandle_GetSourceType(m_handles[i]);
				graphviz_define_metric( name, mode , srct );
			}
		}
	}
}
...
static int64_t find_uber_stride(struct temporal_event_flow * flow)
{
	...
	qsort(flow->additional_first_enters, flow->nr_additional_first_enters,
			sizeof(uint64_t), compare);
	for (uint64_t index = 0; index < flow->nr_additional_first_enters - 1; index++)
	{
		if (diff
				!= (int64_t)(flow->additional_first_enters[index + 1]
												  - flow->additional_first_enters[index]))
		{
			return -1;
		}
	}
 ...
 */
static void graphviz_create_location( const struct SCOREP_Location*            location,
                         __attribute__((unused)) const struct SCOREP_Location* parent_location )
{
	struct thread_info * ti = calloc( 1 , sizeof(struct thread_info) );
	if ( ti == NULL )
		exit( ENOMEM );
	...
	functions->SCOREP_Location_SetData( location , substrate_id , ti );
}

/**
 * Called whenever a location is deleted.
 */
static void graphviz_delete_location( const struct SCOREP_Location* location )
{
	free ( functions->SCOREP_Location_GetData( location , substrate_id ) );
}
...
SCOREP_SUBSTRATE_PLUGIN_ENTRY( graphviz)
{
    ...
    info.create_location       = graphviz_create_location;
    info.delete_location       = graphviz_delete_location;
